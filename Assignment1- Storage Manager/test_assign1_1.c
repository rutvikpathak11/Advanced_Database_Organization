...
static void testWritePageContent(void);
...
main (void)
{
  ...
  //Additional test case: Check the block write function
  testWritePageContent();
...
testWritePageContent(void)
{
	...
	for(i=0; i<PAGE_SIZE; i++)
		ph[i] = (i%10) + '0';
	TEST_CHECK(writeBlock(0,&fh,ph));
	printf("Successful write to the first block");

	//To check write to current position of the file
	TEST_CHECK(writeCurrentBlock(&fh,ph));
	printf("Successful write to the current block");
